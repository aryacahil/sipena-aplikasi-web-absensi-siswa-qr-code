                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            <a href="{{ route('guru.qrcode.show', $session->id) }}" 
                                               class="btn btn-sm btn-info" title="Lihat Detail">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                            
                                            @if($session->status == 'active')
                                            <form action="{{ route('guru.qrcode.close', $session->id) }}" 
                                            ...
                                            @else
                                            <form action="{{ route('guru.qrcode.reopen', $session->id) }}" 
                                                  method="POST" class="d-inline">
                                                @csrf
                ...
                <div class="modal-body">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" id="kelas_id" name="kelas_id" required>
                                ...
                                @foreach($dataKelas as $k)
                                    <option value="{{ $k->id }}">
                                        {{ $k->nama_kelas }} - {{ $k->jurusan->nama_jurusan }} ({{ $k->siswa->count() }} siswa)
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="keterangan" name="keterangan" rows="2" 
                                  placeholder="Contoh: Presensi Mata Pelajaran Matematika"></textarea>
                    </div>
                </div>
...
<script>
// Detect GPS location when modal opens
        ...
        navigator.geolocation.getCurrentPosition(
            function(position) {
                ...
                locationStatus.innerHTML = `
                    ...
                    <small>Akurasi: ${Math.round(position.coords.accuracy)} meter</small>
                `;
...
document.getElementById('createQRModal').addEventListener('hidden.bs.modal', function() {
    document.getElementById('createQRForm').reset();
    document.getElementById('location-status').className = 'alert alert-warning';
    document.getElementById('location-status').innerHTML = '<i class="bi bi-geo-alt me-2"></i>Menunggu deteksi lokasi...';
...
// Validate form
document.getElementById('createQRForm').addEventListener('submit', function(e) {
